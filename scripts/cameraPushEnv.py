...
class CameraEnv(object):
	...
	def __robot_state_callback(self, msg):
		...
		eeX = 3*(self.robot_state.O_T_EE[12]-0.35)
		eeY = 6*self.robot_state.O_T_EE[13]
		self.ee_history[2:10] = self.ee_history[0:8]
		self.ee_history[0] = eeX
		self.ee_history[1] = eeY
	...
	def infer(self):
		...
		while not rospy.is_shutdown():
			if self.depth_rect is not None:
				...
				self.depth_binned = block_reduce(self.depth_normalized, block_size=(3,3), func=np.max, cval=0.0)
				self.depth_binned = np.rot90(self.depth_binned, k=1, axes=(1,0))
				self.depth_binned = np.hstack((np.zeros((150,5)), self.depth_binned[:,:145]))
				...
				action_pred /= 10  # scaling
				# print(action_pred)
				if action_pred[0] > 0.025:
					action_pred[0] = 0.025
